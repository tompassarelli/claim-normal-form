...
import time
from workflow import PRIORITIES, auto_escalates, requires_senior
...
def create_ticket(title: str, description: str, contact_email: str = "",
    ...
    if priority not in PRIORITIES:
        raise ValueError(f"Invalid priority '{priority}'. Must be one of: {sorted(PRIORITIES)}")
    tid = f"T-{_next_ticket[0]}"
    _next_ticket[0] += 1
    now = _now()
    status = "escalated" if auto_escalates(priority) else "open"
    ...
    if requires_senior(priority):
        _auto_assign_senior(t)
    return t


def _auto_assign_senior(ticket: Ticket) -> None:
    candidates = list_users(role="senior") + list_users(role="admin")
    if candidates:
        ticket.assignee = candidates[0].id
