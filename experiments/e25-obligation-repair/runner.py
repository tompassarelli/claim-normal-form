#!/usr/bin/env python3
"""E25: Graph-Native Repair Loop — obligation discovery + repair.

3 agents build ClaimDesk modules simultaneously.
Four conditions:
  file_first:  file agents, no repair (baseline)
  file_repair: file agents + repair agent
  cnf_facade:  facade only, no repair (reproduces E24b)
  cnf_repair:  facade + finish_check + obligation repair

Usage:
    python runner.py --condition cnf_repair
    python runner.py --condition cnf_facade
    python runner.py --condition file_repair
    python runner.py --condition file_first --runs 3
"""
...
FACADE_TOOL_NAMES = [
    ...
    "dependencies", "verify_references", "finish_check",
]
...
# ════════════════════════════════════════════════════════════════════
# Graph-native finish_check (runner-side, calls facade tool via daemon)
# ════════════════════════════════════════════════════════════════════

def call_finish_check(task, code, module_name=""):
    """Call finish_check on the facade MCP server via the daemon."""
    sock = socket.socket()
    sock.connect(("localhost", DAEMON_PORT))

    facade_py = str(SCRIPT_DIR / "facade-tools.py")
    facade_proc = subprocess.Popen(
        [sys.executable, facade_py, str(DAEMON_PORT)],
        stdin=subprocess.PIPE, stdout=subprocess.PIPE,
        stderr=subprocess.PIPE, text=True,
    )

    def facade_rpc(method, params):
        msg = json.dumps({"jsonrpc": "2.0", "id": 1, "method": method,
                          "params": params}) + "\n"
        facade_proc.stdin.write(msg)
        facade_proc.stdin.flush()
        line = facade_proc.stdout.readline()
        return json.loads(line) if line.strip() else {}

    facade_rpc("initialize", {
        "protocolVersion": "2024-11-05",
        "capabilities": {},
        "clientInfo": {"name": "e25-finish-check", "version": "0.1"},
    })

    resp = facade_rpc("tools/call", {
        "name": "finish_check",
        "arguments": {"task": task, "code": code, "module": module_name},
    })
    ...
    try:
        ...
        return {"status": "error", "obligations": [], "error": text}


OBLIGATION_REPAIR_PROMPT = """\
Your code has been checked against the codebase and has missing obligations.
...
The verification found these issues:

{obligations}

Fix your code to address ALL obligations listed above. Use the exact imports \
and checks described. Do NOT change the API signatures.

Write ONLY the corrected {module}. Output ONLY valid Python code."""


def launch_obligation_repair(agent_spec, code, obligations_json, mcp_config_path):
    """Re-launch an agent with its code + obligations to produce a fix."""
    name = agent_spec["name"]
    module = agent_spec["module"]

    obligations_text = json.dumps(obligations_json.get("obligations", []), indent=2)

    prompt = OBLIGATION_REPAIR_PROMPT.format(
        module=module, code=code, obligations=obligations_text)
    ...
    try:
        ...
        output_text = result.stdout or ""
        agent_result = _parse_agent_output(output_text)
        fixed_code = extract_code(agent_result)
        if fixed_code:
            return {"name": name, "code": fixed_code, "elapsed": elapsed,
                    "raw_output": output_text, "error": None}
        else:
            return {"name": name, "code": None, "elapsed": elapsed,
                    "raw_output": output_text,
                    "error": f"No code extracted from repair ({len(output_text)} chars)"}
    except subprocess.TimeoutExpired:
        return {"name": name, "code": None, "elapsed": time.monotonic() - start,
                "raw_output": "", "error": "timeout"}
    except Exception as e:
...
def _launch_cnf_agents(mcp_config_path):
    """Launch all 3 CNF agents in parallel. Returns list of results."""
...
def _launch_file_agents():
    """Launch all 3 file agents in parallel. Returns list of results."""
    print(f"\n  Launching {len(AGENTS)} agents in parallel...", flush=True)
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(AGENTS)) as pool:
        futures = {
...
def run_cnf_facade():
    """CNF facade only, no repair. Reproduces E24b cnf result."""
    w = 72
    print(f"\n{'=' * w}")
    print(f"  E25: cnf_facade")
    print(f"{'=' * w}")

    mcp_config_path = write_mcp_config()
    build_start = time.monotonic()
    results = _launch_cnf_agents(mcp_config_path)
    build_time = time.monotonic() - build_start
    return _finish_condition("cnf_facade", results, build_time, repair_rounds=0)


def run_cnf_repair():
    """CNF facade + finish_check + obligation repair loop."""
    w = 72
    print(f"\n{'=' * w}")
    print(f"  E25: cnf_repair")
    print(f"{'=' * w}")
    ...
    first_pass_info_gap = _count_info_gap(first_pass_results)
    ...
    # Obligation check + repair (up to 2 rounds)
    agent_lookup = {a["name"]: a for a in AGENTS}
    repair_rounds = 0
    for round_num in range(2):
        any_obligations = False
        for r in results:
            if not r.get("code"):
                continue
            agent = agent_lookup[r["name"]]
            task_desc = agent["prompt"].split("{discovery_instruction}")[0].strip()

            check = call_finish_check(task_desc, r["code"], r["module"])
            n_obligations = check.get("obligations_found", 0)
            critical = check.get("critical", 0)

            if n_obligations > 0:
                any_obligations = True
                print(f"\n  finish_check({r['name']}): {n_obligations} obligations"
                      f" ({critical} critical)")
                for ob in check.get("obligations", []):
                    print(f"    [{ob.get('severity', '?')}] {ob.get('domain', '?')}: "
                          f"{ob.get('reason', '')[:80]}")

                if critical > 0:
                    print(f"  Repairing {r['name']}...", flush=True)
                    repair_result = launch_obligation_repair(
                        agent, r["code"], check, mcp_config_path)
                    if repair_result["code"]:
                        r["code"] = repair_result["code"]
                        r["elapsed"] += repair_result["elapsed"]
                        print(f"    Repair done ({repair_result['elapsed']:.1f}s)")
                    else:
                        print(f"    Repair failed: {repair_result.get('error', '?')}")
            else:
                print(f"  finish_check({r['name']}): pass")
    ...
    build_time = time.monotonic() - build_start
    result = _finish_condition("cnf_repair", results, build_time, repair_rounds)
...
def run_file_first():
    """File agents, no repair. Baseline."""
    w = 72
    print(f"\n{'=' * w}")
    print(f"  E25: file_first")
    print(f"{'=' * w}")

    build_start = time.monotonic()
    results = _launch_file_agents()
    build_time = time.monotonic() - build_start
    return _finish_condition("file_first", results, build_time, repair_rounds=0)


def run_file_repair():
    """File agents + repair agent. E24b's strong baseline."""
    w = 72
    print(f"\n{'=' * w}")
    print(f"  E25: file_repair")
    print(f"{'=' * w}")

    build_start = time.monotonic()
    results = _launch_file_agents()

    out_dir = SCRIPT_DIR / "file_repair"
    ...
    build_time = time.monotonic() - build_start
    result = _finish_condition("file_repair", results, build_time, repair_rounds)
    result["first_pass_info_gap"] = first_pass_info_gap
    result["first_pass_tests"] = first_pass_results
    return result


def _count_info_gap(test_results):
    """Count info-gap test failures."""
    count = 0
...
def aggregate_multi(all_runs, condition):
    ...
    w = 72
    print(f"\n{'=' * w}")
    print(f"  E25-multi: {condition} — {n} runs")
    print(f"{'=' * w}")
...
CONDITION_RUNNERS = {
    "cnf_facade": run_cnf_facade,
    "cnf_repair": run_cnf_repair,
    "file_first": run_file_first,
    "file_repair": run_file_repair,
}

NEEDS_DAEMON = {"cnf_facade", "cnf_repair"}


def main():
    parser = argparse.ArgumentParser(description="E25: Graph-Native Repair Loop")
    parser.add_argument("--condition", default="cnf_repair",
                        choices=list(CONDITION_RUNNERS.keys()),
                        help="Which condition to run (default: cnf_repair)")
    ...
    results_path = SCRIPT_DIR / f"results-{condition}.json"
    output = {
