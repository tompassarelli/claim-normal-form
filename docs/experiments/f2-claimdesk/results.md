...
CNF eliminates this by giving agents a shared structural model. Agent 4
queries "does an archived state exist?" before writing notification
code. The answer comes from the graph — not from reading files, not
from grepping, not from guessing. The structural knowledge propagates
...
- **Scripted agents.** The agents are deterministic scripts, not LLMs.
  A real LLM agent in the git condition might grep for "archive" and
  discover the workflow module. But it would have to read the file,
  understand the state machine, and infer the implication for its own
  feature — exactly the "private cognition" problem. The CNF agent
  skips all of that: one query, one answer.
